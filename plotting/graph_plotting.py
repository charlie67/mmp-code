...
def plot_nodes(array, program_options: Options):
    for node in array:
        plt.plot(node[0], node[1], 'b.', markersize=4)
    plt.title(program_options.TSP_PROBLEM_NAME + " All nodes")
    plt.savefig(program_options.OUTPUT_DIRECTORY + program_options.TSP_PROBLEM_NAME + "-all-nodes.png",
...
def plot_aco_clustered_tour(tour, clustered_data, program_options: Options):
    ...
    plt.savefig(program_options.OUTPUT_DIRECTORY + program_options.TSP_PROBLEM_NAME + "-aco-clustered-tour.png",
                dpi=program_options.PLT_DPI_VALUE)
    plt.close()
...
def plot_complete_tsp_tour(tour, node_id_to_coordinate_dict, title, program_options: Options,
    ...
    for i in tour:
        ...
        num += 1
    plt.title(title)
    plt.savefig(program_options.OUTPUT_DIRECTORY + program_options.TSP_PROBLEM_NAME + title + "-solution.png",
                dpi=program_options.PLT_DPI_VALUE)
    if program_options.DISPLAY_PLOTS:
        plt.show()
